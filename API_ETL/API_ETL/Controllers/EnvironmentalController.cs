...
//Date: August 2, 2022
//Purpose: The EnvironmentalController handles the the GET request (including optional parameters), forms the SQL query string, passes it to SQLHandler for the request to be completed,
//and then returns the result, all for the Environmental table.
...
namespace API_ETL.Controllers
{
    ...
    public class EnvironmentalController : Controller
    {
        ...
        public string getParameters([FromQuery] string? StartDate, string? EndDate, int? DepartmentID)
        {
            ...
            sqlRequest += "SELECT TOP 500 * FROM [DataHub_OESSHE].[IMPACT].[VW_PER_ENVIRONMENTAL]";
            bool hasParam = false;
