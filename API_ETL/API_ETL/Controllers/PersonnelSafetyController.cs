...
//Date: August 2, 2022
//Purpose: The PersonnelSafetyController handles the the GET request (including optional parameters), forms the SQL query string, passes it to SQLHandler for the request to be completed,
//and then returns the result, all for the Personnel Safety table.
...
namespace API_ETL.Controllers
{
    ...
    [ApiController]
    public class PersonnelSafetyController : Controller
    {
        ...
        public string getParameters([FromQuery] string? StartDate, string? EndDate, int? DepartmentID)
        {
            ...
            sqlRequest += "SELECT TOP 500 * FROM [DataHub_OESSHE].[IMPACT].[VW_PER_PERSONNEL_SAFETY]";
            bool hasParam = false;
